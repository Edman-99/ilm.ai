...
Builds an optimal investment portfolio from scratch. Claude selects instruments from the full S&P 500 + ETF universe using BlackRock methodology, then fetches live prices. Requires `pro` or `premium` plan.
...
| `allocations[].ticker` | string | Ticker symbol |
| `allocations[].name` | string | Full instrument name |
| `allocations[].asset_class` | string | Asset class / GICS sector |
...
| `allocations[].price` | float | Live price at time of request |
...
| 503 | `{"error": "marketDataUnavailable"}` | All selected tickers failed price fetch |
